...
class VAEImageMechanism(ImageMechanism):

    def __init__(self, model: ConditionalVAE, deterministic: bool = False):
        self.model = model
        self.deterministic = deterministic

    def abduct(self, obs: Dict) -> Dict:
        image, cond = obs["image"], obs["metadata"].float()
        z, u = self.model.encode(image, cond, self.deterministic)
        return {"z": z, "u": u}
